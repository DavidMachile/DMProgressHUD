...
@class DMBackgroundView;
@protocol DMProgressHUDDelegate;


extern CGFloat const DMProgressMaxOffset;

typedef NS_ENUM(NSInteger, DMProgressHUDMode) {
    /// UIActivityIndicatorView.
    DMProgressHUDModeIndeterminate,
    /// A round, pie-chart like, progress view.
    DMProgressHUDModeDeterminate,
    /// Horizontal progress bar.
    DMProgressHUDModeDeterminateHorizontalBar,
    /// Ring-shaped progress view.
    DMProgressHUDModeAnnularDeterminate,
    /// Shows a custom view.
    DMProgressHUDModeCustomView,
    /// Shows only labels.
...
typedef NS_ENUM(NSInteger, DMProgressHUDAnimation) {
    /// Opacity animation
    DMProgressHUDAnimationFade,
    /// Opacity + scale animation (zoom in when appearing zoom out when disappearing)
    DMProgressHUDAnimationZoom,
    /// Opacity + scale animation (zoom out style)
    DMProgressHUDAnimationZoomOut,
    /// Opacity + scale animation (zoom in style)
    DMProgressHUDAnimationZoomIn
};

typedef NS_ENUM(NSInteger, DMProgressHUDBackgroundStyle) {
    /// Solid color background
    DMProgressHUDBackgroundStyleSolidColor,
    /// UIVisualEffectView or UIToolbar.layer background view
    DMProgressHUDBackgroundStyleBlur
};

typedef void (^DMProgressHUDCompletionBlock)(void);
 ...
 * The DMProgressHUD window spans over the entire space given to it by the initWithFrame: constructor and catches all
 * user input on this region, thereby preventing the user operations on components below the view.
 *
 * @note To still allow touches to pass through the HUD, you can set hud.userInteractionEnabled = NO.
 * @attention DMProgressHUD is a UI class and should therefore only be accessed on the main thread.
 */
 ...
 */
+ (nullable DMProgressHUD *)HUDForView:(UIView *)view;
 ...
 */
@property (weak, nonatomic) id<DMProgressHUDDelegate> delegate;

/**
 * Called after the HUD is hiden.
 */
@property (copy, nullable) DMProgressHUDCompletionBlock completionBlock;
...
/**
 * DMProgressHUD operation mode. The default is DMProgressHUDModeIndeterminate.
 */
@property (assign, nonatomic) DMProgressHUDMode mode;
 ...
 */
@property (assign, nonatomic) DMProgressHUDAnimation animationType UI_APPEARANCE_SELECTOR;

/**
 * The bezel offset relative to the center of the view. You can use DMProgressMaxOffset
 * and -DMProgressMaxOffset to move the HUD all the way to the screen edge in each direction.
 * E.g., CGPointMake(0.f, DMProgressMaxOffset) would position the HUD centered on the bottom edge.
 */
 ...
 */
@property (strong, nonatomic, readonly) DMBackgroundView *bezelView;

/**
 * View covering the entire HUD area, placed behind bezelView.
 */
@property (strong, nonatomic, readonly) DMBackgroundView *backgroundView;

/**
 * The UIView (e.g., a UIImageView) to be shown when the HUD is in DMProgressHUDModeCustomView.
...
@protocol DMProgressHUDDelegate <NSObject>
 ...
 */
- (void)hudWasHidden:(DMProgressHUD *)hud;
...
@interface DMBackgroundView : UIView

/**
 * The background style.
 * Defaults to DMProgressHUDBackgroundStyleBlur on iOS 7 or later and DMProgressHUDBackgroundStyleSolidColor otherwise.
 * @note Due to iOS 7 not supporting UIVisualEffectView, the blur effect differs slightly between iOS 7 and later versions.
 */
@property (nonatomic) DMProgressHUDBackgroundStyle style;

#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000 || TARGET_OS_TV
/**
 * The blur effect style, when using DMProgressHUDBackgroundStyleBlur.
...
@interface DMProgressHUD (Deprecated)
...
- (void)showAnimated:(BOOL)animated whileExecutingBlock:(dispatch_block_t)block completionBlock:(nullable DMProgressHUDCompletionBlock)completion __attribute__((deprecated("Use GCD directly.")));
...
- (void)showAnimated:(BOOL)animated whileExecutingBlock:(dispatch_block_t)block onQueue:(dispatch_queue_t)queue
     completionBlock:(nullable DMProgressHUDCompletionBlock)completion __attribute__((deprecated("Use GCD directly.")));
...
@property (strong, nonatomic) UIColor *activityIndicatorColor __attribute__((deprecated("Use UIAppearance to customize UIActivityIndicatorView. E.g.: [UIActivityIndicatorView appearanceWhenContainedIn:[DMProgressHUD class], nil].color = [UIColor redColor];")));
